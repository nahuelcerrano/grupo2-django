...
@app.route('/')
def index():
    return render_template('empleados/z_inicial.html')
...
def storage():
    ...
    return redirect('/z_listar')       # Volvemos a index.html

#DESDE ACA COLOCO TODAS LAS FUNCIONES NUEVAS
#--------------------------------------------------------------------
# Función para volver al nodo raiz. se usa en el boton del menu listar empleados
@app.route('/z_listar')
def z_listar():
#    def index():
    ...
    return render_template('empleados/z_listar.html', empleados = db_empleados)
...
@app.route('/z_crear')
def z_crear():
    return render_template('empleados/z_crear.html')

#--------------------------------------------------------------------
# Función para mostrar el form acerca de se usa en el pie.
@app.route('/z_acerca')
def z_acerca():
    return render_template('empleados/z_acerca.html')

#--------------------------------------------------------------------
# Función para editar un registro
@app.route('/z_editar/<int:id>')
def editar(id):
    conn = mysql.connect()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM `sistema`.`empleados` WHERE id=%s", (id))
    empleados=cursor.fetchall()
    conn.commit()
    return render_template('empleados/z_editar.html', empleados=empleados)
#--------------------------------------------------------------------
# Función para eliminar un registro
@app.route('/z_borrar/<int:id>')
def z_borrar(id):
    conn = mysql.connect()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM `sistema`.`empleados` WHERE id=%s", (id))
    empleados=cursor.fetchall()
    conn.commit()
    return render_template('empleados/z_borrar.html', empleados=empleados)
